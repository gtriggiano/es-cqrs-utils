...
const AggregateSavingError = exports.AggregateSavingError = (0, _utils.DefineError)('AggregateLoadingError');
  ...
  return Object.defineProperties(repository, {
    load: { value: aggregates => {
        ...
        return Promise.all(aggregates.map(aggregate => {
          let loadSnapshot = snapshotService ? snapshotService.loadSnapshot(aggregate.snapshotKey).catch(() => null) : Promise.resolve(null);
    ...
    save: { value: aggregates => {
        ...
        return eventstoreService.saveEventsToMultipleStreams(aggregatesToSave.map(aggregate => ({
          stream: aggregate.stream,
          events: aggregate.newEvents.map(({ type, serializedData }) => ({ type, data: serializedData })),
          expectedVersion: aggregate.persistenceConsistencyPolicy === _AggregateFactory.ENSURE_VERSION_CONSISTENCY ? aggregate.version : aggregate.persistenceConsistencyPolicy === _AggregateFactory.AGGREGATE_SHOULD_EXIST ? -1 : -2
        }))).then(() => repository.load(aggregates)).catch(eventStoreError => {
...
const _validateEventstoreServiceInterface = exports._validateEventstoreServiceInterface = eventstoreService => {
  if (!(0, _isObject2.default)(eventstoreService)) throw new TypeError('eventstoreService MUST be an object like {getEventsOfStream(), saveEventsToMultipleStreams()}');
  ...
  if (!(0, _isFunction2.default)(eventstoreService.saveEventsToMultipleStreams)) throw new TypeError('eventstoreService.saveEventsToMultipleStreams([{stream, events, consistencyPolicy}, ...]) MUST be a function');
};
